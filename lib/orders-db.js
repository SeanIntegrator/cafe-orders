...
async function insertOrderWithItems(client, params) {
  ...
  const ins = await client.query(
    `INSERT INTO orders (square_order_id, user_id, customer_name, notes, allergens, total_amount, status, order_source, pickup_time, cafe_id, stripe_session_id)
...
async function insertPendingOrder({
  ...
  const { rows } = await pool.query(
    `INSERT INTO pending_orders (user_id, line_items, customer_name, pickup_minutes, notes, allergens, total_amount)
...
async function fetchOpenWebAppOrdersWithItems() {
  const { rows: orderRows } = await pool.query(
    `SELECT id, square_order_id, customer_name, notes, allergens, total_amount, status, pickup_time, created_at, updated_at, order_source
     FROM orders

...
const { kdsShouldDisplayOrder, isOrderPaid } = require('./kds-visibility');
...
/**
 * OPEN and COMPLETED orders with updated_at on or after startAtIso (RFC 3339).
 * KDS poller safety net: COMPLETED covers paid-at-register orders where fulfillments are still active
 * (filtered client-side via kdsShouldDisplayOrder). Pulls more rows than OPEN-only; limit 100 per poll.
 * @param {string} startAtIso
 * @returns {Promise<object[]>}
 */
async function searchKdsPollOrdersUpdatedSince(startAtIso) {
  requireToken();
  const locationId = await getLocationId();
  if (!locationId) {
    console.error('No Square location found');
  ...
  if (!startAtIso || typeof startAtIso !== 'string') {
    console.error('searchKdsPollOrdersUpdatedSince: invalid startAtIso');
    return [];
  }
  const searchBody = {
    location_ids: [locationId],
  ...
  if (!res.ok) {
    const text = await res.text();
    console.error('SearchOrders (KDS poll, updated_since) failed:', res.status, text);
    return [];
  }
  const data = await res.json();
  return data.orders ?? [];
...
/**
 * Fetch all Square orders with optional filters. No default state/fulfillment
 * restrictions — includes POS orders, any status, any date.
 *
 * @param {object} [opts]
 * @param {string[]} [opts.states]       - e.g. ['OPEN','COMPLETED','CANCELED']. Omit for all.
 * @param {string}   [opts.createdAfter] - RFC 3339 lower bound on created_at
 * @param {string}   [opts.createdBefore]- RFC 3339 upper bound on created_at
 * @param {number}   [opts.limit=100]
 * @param {string}   [opts.cursor]       - pagination cursor from previous response
 * @returns {Promise<{ orders: object[], cursor?: string }>}
 */
async function searchAllOrders({ states, createdAfter, createdBefore, limit = 100, cursor } = {}) {
  requireToken();
  const locationId = await getLocationId();
  if (!locationId) return { orders: [] };

  const filter = {};
  if (states?.length) filter.state_filter = { states };
  if (createdAfter || createdBefore) {
    filter.date_time_filter = {
      created_at: {
        ...(createdAfter && { start_at: createdAfter }),
        ...(createdBefore && { end_at: createdBefore }),
      },
    };
  }
  ...
  const body = {
    location_ids: [locationId],
    limit,
    // Square requires sort_field to match the date_time_filter field used
    ...(filter.date_time_filter && { sort: { sort_field: 'CREATED_AT', sort_order: 'DESC' } }),
  ...
  if (!res.ok) {
    const text = await res.text();
    console.error('searchAllOrders failed:', res.status, text);
    return { orders: [] };
  }
  const data = await res.json();
...
/**
 * Lower bound on created_at for KDS board Square search (RFC 3339).
 * Default: start of current UTC day — same as history modal "today".
 * Override: KDS_BOARD_CREATED_SINCE_ISO
 */
function kdsBoardCreatedAfterIso() {
  const raw = process.env.KDS_BOARD_CREATED_SINCE_ISO;
...
/**
 * OPEN + COMPLETED orders created on or after the board window start.
 * Env: KDS_BOARD_CREATED_SINCE_ISO, KDS_BOARD_SEARCH_LIMIT (1–500, default 100).
 * @returns {Promise<object[]>}
 */
async function searchKdsBoardSquareOrders() {
  const createdAfter = kdsBoardCreatedAfterIso();
  const limit = Math.min(
    Math.max(1, parseInt(process.env.KDS_BOARD_SEARCH_LIMIT || '100', 10) || 100),
    500
  );
...
module.exports = {
  ...
  searchOpenOrders,
  searchKdsBoardSquareOrders,
  searchKdsPollOrdersUpdatedSince,
  searchAllOrders,

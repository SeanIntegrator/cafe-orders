...
async function listCatalogItems() {
  requireToken();
  const res = await fetch(`${SQUARE_BASE_URL}/v2/catalog/list?types=ITEM,CATEGORY`, {
    method: 'GET',
    headers: squareHeaders,
  });
  if (!res.ok) {
  ...
  for (const obj of objects) {
    ...
    if (variations?.length && variations[0].id && name) {
      ...
      const categoryId = obj.item_data?.categories?.[0]?.id ?? obj.item_data?.category_id ?? null;
      const categoryName = categoryId ? (categoryMap[categoryId] ?? null) : null;
      items.push({ id: variations[0].id, name, price: priceAmount, categoryName });
    }
  }
  return items;
}
